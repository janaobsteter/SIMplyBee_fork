...
honeybee biology. Specifically, it describes:
  ...
  1. initiating simulation with genomes and parameters,
...
# Initiating simulation with genomes and parameters
...
We will use the other virging queen to create a colony. In SIMplyBee a colony is
...
We build up colony with workers and drones with `buildUpColony()`. 
...
disseminate queen's genes, and one of virgin queens will eventually replace the
queen. We also store fathers, which represent drones that the queen mated with.
Storing fathers enables us to generate colony members on demand. Let's count how
many individuals we have for each caste.
