...
In these following examples, we will be demonstrating `getColony*()` and `getCaste*()`, however be aware that 
there are also functions that can be used on the individual caste members level (for example: `getQueenSNPhaplo()`).
We would recommend studying the index of `get*()` functions to familiarize yourself with all options available for your data collection. 


Firstly, let's build up the colony to have a bit data to look at. By using the default, we are adding 100 workers and 100 drones into the colony.
```{r buildUp colony}
colony <- buildUp(colony, nWorkers = 10, nDrones = 10)
```

Only `get*SegSites()` will be demonstrated in this vignette as `get*Qtl()` and `get*SNP()` allow us to extract from the haplotypes and the genotypes in the exact same way. Whereas, `get*Ibd()` is only performed on the haplotypes but has the same functionality. 


Lets look at the whole colony first, this will produce quite a large list of
```{r Colony SegSites}
colonySegSitesGeno <- getColonySegSiteGeno(colony)
colonySegSitesHaplo <- getColonySegSiteHaplo(colony)
```

Now lets have a look at the queen using `getCasteSegSites*()`. Remember that you can also use the `getQueenSegSites*()` to 
produce the same result
```{r Colony SegSites}
queenSegSitesGeno <- getCasteSegSiteGeno(x = colony, caste = "queen")
queenSegSitesHaplo <- getCasteSegSiteHaplo(x = colony, caste = "queen")
```
