...
honeybees is versatile. Queens can be mated in a controlled manner, either in
artificial insemination or on a mating station, or uncontrolled in an open
mating setting. In the latter case, the mating depends on the distance between
the queen and the drones or drone producing colonies.

In this vignette, we first explain five technical ways of mating a virgin queens
and at the end we describe two special cases of mating the queen on a mating
station and mating multiple queens in varyin manner.

```{r crossing_decision_tree, echo=FALSE, out.width='50%', fig.cap = "Crossing decision tree"}
knitr::include_graphics("../man/figures/decisionTreeCrossing.png")
```
...
-   **create the cross plan internally**, either by sampling the mates randomly
    or spatially. Continue to question 4.
...
Here, we show an example for mating the virgin queen in a `Pop` object, but
exact same holds for `Colony` and `MultiColony` objects with a single queen.
...
queens1 <- cross(virginQueens1, drones = droneGroups[1:3])
nFathers(queens1)
```
...
```{r}
o# Create a single drone for single drone insemination
...
crossPlanBeekeeper7 <- c(
  createRandomCrossPlan(IDs = getId(beekeeper7)[1], 
                        drones = singleDrone, 
                        nDrones = 1),
  createRandomCrossPlan(IDs = getId(beekeeper7)[2:6], 
                        drones = DCA, 
                        nDrones = nFathersTruncPoisson),
  createRandomCrossPlan(IDs = getId(beekeeper7)[7:10], 
                        drones = matingStationDCA, 
                        nDrones = nFathersTruncPoisson)
  )

beekeeper7 <- cross(beekeeper7, 
