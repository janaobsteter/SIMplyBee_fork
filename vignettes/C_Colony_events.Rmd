...
that leave with the swarm. The function returns is a list with two `Colony`
class objects, `swarm` and `remnant`. The `swarm` contains the old queen and `p`
...
(Figure 2). Argument `p` is proportion of workers that will go to new colony.
The output of the function is a list of two `Colony` or `MultiColony` objects.
The `split()` function follows the same principles as the `swarm()`, hence we
will spent less time explaining the outputs.
...
```{r create the colony and Multicolony }
