...
example when mating virgin queens.
...
from an uniform distribution with the mean of 0.6. For this, we use
`swarmPUnif()` function that takes a `min` and a `max` values and sample a value
between them. By default, the `min` is set to 0.4 and `max` to 0.6. Let's use
this function to sample a 1000 values between 0.5 and 0.7 and inspect the mean.
