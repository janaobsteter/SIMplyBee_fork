...
age of the queens or geographical location of the apiaries. First, we again load
...
supersedure, or a collapse (you can read more about this events in the Colony
events vignette), and how many of them are productive, meaning that we can
collect a production phenotype from them (i.e. honey).
...
as the input parameter. Let's call this apiary `apiary1` and say that it stands
at the location 1 (1,1).
...
These three funtions can also select, pull, and remove colonies based on some
...
Let's now initiate another `MultiColony` names `apiary2` that is placed at
location `(2,2)`. Here, we define different `MultiColony` objects according to
the location of the apiary, but the objects could also be defined according to
the age of the queens - `age0`, `age1`... `apiary2` contains only virgin queens
and we want to mate them to a DCA made of drones in `apiary1`.
