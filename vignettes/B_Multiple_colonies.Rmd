...
output:
  ...
  html_document:
    df_print: paged
vignette: >
  %\VignetteIndexEntry{MultipleColonies}
...
populations instead. To cater for this, we created a `MultiColony` class. It
behaves as a list of `Colony` objects but with additional functionality - you
can apply function directly to the `MultiColony` objects. A `MultiColony` can
represent different apiaries or even different subpopulations in terms of either
age of the queens of geographical location of the apiaries. First, we again load
the package, if we haven't already.
...
empty apiary without any colonies and individuals within.
...
Let's inspect the printout of the `MultiColony` object. This tells use how many
colonies within it are `populated`, meaning they contain some individuals, how
many are `empty` and contain no individuals, how many are `NULL` objects, how
many have experienced a split, swarm, supersedure, or a collapse (you can read
more about this events in the Colony events vignette), and how many of them are
productive, meaning that we can collect a production phenotype from them (i.e.
honey).
...
virgin or mated queens. For this, we first have to initialise the simulation by
simulating founder genomes and creating a base population of virgin queens. We
will use 10 virgin queens to produce drones and create a DCA.
...
`createMultiColony()` function by providing the remaining 10 virgin queens as
the input parameter. Let's say this apiary stands at the location 1 (1,1) and
call it `apiary1`.
...
Once we have a non-empty `MultiColony` object, we can do some basic operations
on it. First, we can select some of colonies by either specifying the IDs,
desired number or percentage of randomly selected colonies.
...
that the pulled colonies are pulled-out and removed from the original object.
The function `pullColonies()` therefore returns two object - the pulled colonies
and the remnant colonies.
...
`pullDroneGroupsFromDCA()` function.
...
Once we have mated queens in the apiary, we can apply all the events functions
directly to the `MultiColony` object: `buildUp()`, `downsize()`, `swarm()`,
`split()`, `supersede()`, `collapse()` but also all the functions that either
add, replace, or remove individuals from the castes. Let's say we know want to
build up all the colonies in our apiary. We can use the `pullColonies()` or
`selectColonies()` to subset the colonies that will for example swarm, collapse,
or supersede (presented in the Colony events vignette), or the ones that we
decided to split (check out the Colony events vignette).
...
Here, we defined different `MultiColony` objects according to the location of
the apiary. The objects could also be defined according to the age of the
queens - `age0`, `age1`... Here, `apiary2` are actually all new virgin queens of
age 0 produced through splitting and swarming.

Let's now initiate another `MultiColony` names `apiary2` that contains only
virgin queens. We want to mate them to a DCA made of drones in `apiary1`.
