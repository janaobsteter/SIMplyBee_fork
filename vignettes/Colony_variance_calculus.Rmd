...
individual honeybee level. The challenge here is that in reality we do not observe
such variances, but only on a colony level.
...
variation, but looking at colony genetic and phenotypic variances the ratio was
aobut 1/2!
...
genetic parameters. Recall that our starting quantitative genetic parameters
for the queen and workers effects were:
...
queen effect $g_{q,q}$ and workers' genetic values for the workers effect $\Sigma_{i=1}^{n_w}(g_{i,w})$:
...
So, we have three parts: A) the variance of the queen effect in queens
$\sigma^2_{g_{q}}$, B) variance of a sum of workers effect in workers
...
Given that this is a sum, we need to look at: B1) variance of each of worker
genetic values $Var(g_{i,w})$ and B2) covariance between each pair of the
...
$g_{2,w} = \frac{1}{2}g_{q,w} + g_{f,w} + r_{2,w}$ ... $g_{n_w,w} = \frac{1}{2}g_{q,w} + g_{f,w} + r_{n_w,w}$
...
drones is in fact half the genetic variance between queens because they are
haploid, so we get:
...
$2\frac{1}{2}\sigma^2_{g_w} = \sigma^2_{g_w}$. With $n_w$ workers we get $n_w n_w$
pairs (including with itself) or $n_w n_w - n_w$ pairs between different workers.
The total covariance contribution is then
...
this is $n_f (n_f - 1) \frac{n_w}{n_f} \frac{n_w}{n_f} = (n_f - 1) n_w \frac{n_w}{n_f} = \frac{ (n_f - 1) n^2_w}{n_f}$. Is this correct?
...
TODO: What if $n_w$ is a random variable? Following <https://en.wikipedia.org/wiki/Distribution_of_the_product_of_two_random_variables> we would use:
...
are essentially this: $\sigma^2_{g_c} = \sigma^2_{g_{c,q}} + \sigma^2_{g_{c,w}} + 2\sigma_{g_{c,q},g_{c,w}}$
where $\sigma^2_{g_{c,q}}$ is the queen part of $\sigma^2_{g_c}$, but not
necessarily the same as $\sigma^2_{g_w}$, or is it? Maybe that is the same,
and $\sigma^2_{g_{c,w}}$ is a counterpart to $n_w \sigma^2_{g_w} + \frac{n^2_w}{n_f} \frac{1}{2} \sigma^2_{g_w}, \frac{(n_f - 1) n^2_w}{n_f} \frac{1}{4} \sigma^2_{g_w}$,
while $\sigma_{g_{c,q},g_{c,w}}$ is a counterpart to $n_w \sigma^2_{{g_q},{g_w}}$.
If this is so, then it fells that we should have 3 more inputs for genetic part,
